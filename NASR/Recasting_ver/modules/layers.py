...
class PoolingExpandLayer(My2DLayer):
    ...
    def __init__(self, in_channels, out_channels,
                 pool_type, kernel_size=2, stride=2, groups=1, bias=False,
                 use_bn=True, act_func='relu', dropout_rate=0, ops_order='weight_bn_act'):
        ...
        super(PoolingExpandLayer, self).__init__(in_channels, out_channels, use_bn, act_func, dropout_rate, ops_order)
    ...
    def config(self):
        return {
            'name': PoolingExpandLayer.__name__,
            'pool_type': self.pool_type,
            'kernel_size': self.kernel_size,
            'stride': self.stride,
            'groups': self.groups,
            'bias': self.bias,
            **super(PoolingExpandLayer, self).config
        }

    @staticmethod
    def build_from_config(config):
        return PoolingExpandLayer(**config)
